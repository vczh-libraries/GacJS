...
export function typeToString(t: Type, classNames: string[], prefix: string = ''): string {
    switch (t['$ast']) {
        ...
        case 'ReferenceType':
            return refToString(t.name, classNames, prefix);
        case 'OptionalType':
            return `${prefix}TYPES.Nullable<${typeToString(t.element, classNames, prefix)}>`;
        case 'ArrayType':
            return `${prefix}TYPES.List<${typeToString(t.element, classNames, prefix)}>`;
        case 'ArrayMapType':
            return `${prefix}TYPES.ArrayMap<${refToString(t.element, classNames, prefix)}, '${t.keyField}'>`;
        case 'MapType':
            return `${prefix}TYPES.Dictionary<${typeToString(t.keyType, classNames, prefix)}, ${typeToString(t.element, classNames, prefix)}>`;
        default:
